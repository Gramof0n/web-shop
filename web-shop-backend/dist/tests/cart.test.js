...
describe("Cart endpoints", () => {
    describe("GET cart route", () => {
        it("should get the cart for user with the ID of 2", async () => {
            const res = await chai_1.default.request(url).get("/cart/2");
    ...
    describe("GET add products to cart", () => {
        it("should add products with the ID of 5 and 4 to the user with the ID of 4", async () => {
            await chai_1.default.request(url).get("/cart/add/user_id=4&product_id=5");
            await chai_1.default.request(url).get("/cart/add/user_id=4&product_id=4");
            const res = await chai_1.default.request(url).get("/cart/4");
    ...
    describe("GET delete product from cart", () => {
        it("should remove a product with the ID of 5 from the user with the ID of 4", async () => {
            const previous_cart = await chai_1.default.request(url).get("/cart/4");
            await chai_1.default.request(url).get("/cart/remove/user_id=4&product_id=5");
            const current_cart = await chai_1.default.request(url).get("/cart/4");
            expect(previous_cart.body.products.length).to.be.greaterThan(current_cart.body.products.length);
        });
    });
    describe("GET clear all from cart", () => {
        it("should remove everything from cart of the user with the ID of 4", async () => {
            await chai_1.default.request(url).get("/cart/clear/4");
            const current_cart = await chai_1.default.request(url).get("/cart/4");
